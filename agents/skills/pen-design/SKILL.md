...
The CLI needs auth to run its AI agent for which Codex is required. For that
there needs to be an authenticated Codex user set in the system configuration
...
| **Codex** | Often `.Codex/skills/pen-design/SKILL.md` or user-level under `~/.Codex/` |
...
- `--model, -m` — Codex model to use (defaults to Opus)
