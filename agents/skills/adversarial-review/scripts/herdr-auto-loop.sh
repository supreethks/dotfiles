...
echo "======================================================"
echo " Starting Autonomous Adversarial Review Loop ($LLM_RUNNER)"
...
while [ "$ROUND" -le "$MAX_ROUNDS" ]; do
  ...
  for i in "${!REVIEWERS[@]}"; do
    ...
    if [ "$LLM_RUNNER" = "agy" ]; then
      (echo "$DIFF" | agy -p "$(cat "$PROMPT_FILE")" --no-context > "$TF" 2>&1 || true) &
...
${FEEDBACK_ITEMS}"
  ...
  if [ -n "${HERDR_SESSION_ID:-}" ]; then
    ...
    echo "$FEEDBACK_PAYLOAD" | agy --continue --non-interactive
  elif [ "$LLM_RUNNER" = "codex" ]; then
